...
% "error_space.mat", which corresponds to initial: Gauss +
% (s,p,gamma,d) = (0.6,1,-1,3).
...
loglog(Line,10^(0.5)*Line.^(-4.02),'r--','LineWidth',2), hold on,
loglog(Line,10^(1.25)*Line.^(-6.23),'b--','LineWidth',2)
...
axis([2^2,2^6,10^-10,10^0.5])

% Legend
h_legend = legend('$L^2$-error','$L^\infty$-error','$\propto N^{-4.02}$','$\propto N^{-6.23}$','Interpreter', 'latex');
