...
% and initial value function \psi_0(x,y,z) = exp(-x^2-y^2-z^2).
...
mu = 1.1/sqrt(N); % the scaling factor for MT-functions, change {MT(j,x)*MT(k,y)*MT(l,z)} to {MT(j,mu*x)*MT(k,mu*y)*MT(l,mu*z)}
