...
loglog(Line,10^(-1.05)*Line.^(-2.53),'r--','LineWidth',2), hold on,
loglog(Line,10^(-0.87)*Line.^(-3.28),'b--','LineWidth',2)
...
set(gca,'yTickLabel',{'10^{-8}','10^{-6}','10^{-4}', ...
    '10^{-2}','10^{0}'});
set(gca,'TickLabelInterpreter','tex');
axis([2^1,2^8,10^-8.8,10^0])

% Legend
h_legend = legend('$L^2$-error','$L^\infty$-error','$\propto N^{-2.53}$','$\propto N^{-3.28}$','Interpreter', 'latex');
