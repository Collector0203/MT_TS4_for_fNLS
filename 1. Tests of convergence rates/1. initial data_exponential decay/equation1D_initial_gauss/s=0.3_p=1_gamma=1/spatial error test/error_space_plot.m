...
loglog(NNN,error_Linf_T,'bp','MarkerSize',10,'LineWidth', 1.0), hold on,
loglog(Line,10^(-1.15)*Line.^(-1.67),'r--','LineWidth',2), hold on,
loglog(Line,10^(-1.28)*Line.^(-2.45),'b--','LineWidth',2)
...
axis([2^1,2^8,10^-7.23,10^0])

% Legend
h_legend = legend('$L^2$-error','$L^\infty$-error','$\propto N^{-1.67}$','$\propto N^{-2.45}$','Interpreter', 'latex');
