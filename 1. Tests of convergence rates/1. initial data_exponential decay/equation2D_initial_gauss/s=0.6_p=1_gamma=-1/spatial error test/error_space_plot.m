...
loglog(Line,10^(-0.35)*Line.^(-3.29),'r--','LineWidth',2), hold on,
loglog(Line,10^(0)*Line.^(-4.77),'b--','LineWidth',2)
...
axis([2^2,2^7,10^-10.1,10^-0])

% legend
h_legend = legend('$L^2$-error','$L^\infty$-error','$\propto N^{-3.29}$','$\propto N^{-4.77}$','Interpreter', 'latex');
