...
% solves this equation with parameters (s,p,gamma) = (0.3,1,1) and initial
% value function \psi_0(x) = 1/(1+x^2)^sqrt(2) by BioMT + TSM4, and
...
NNN = ceil(2.^(1:0.5:8)); % truncation, \psi(x,t) = sum(n=-N:N-1) a(n,t)*MT(n,mu*x)
Mu = 0.6./sqrt(NNN); % the scaling factor for MT-functions, change {MT(n,x)} to {MT(n,mu*x)}
dt = 0.01; % time step, t_{n+1} = t_n + dt
...
Unum_coeff = cell(length(NNN),1); % Unum_coeff{k,1} denotes the MT-coefficients of the numerical solution \psi(x,T) corresponding to NNN(k) and Mu(k)
M = 500; % test interval [-M,M]
pt = -M:M/2000:M; % test points pt
Unum_valpt = zeros(length(pt),length(NNN)); % Unum_valpt(:,k) denotes the values of the numerical solution \psi(x,T) corresponding to NNN(k) and Mu(k) on test points pt
...
for N = NNN
    ...
    node_mu = node/mu; % MT-nodes of mu (colunm vector)
    ...
    [NN,ppt] = meshgrid(-N:N-1,pt); MTmatrix = MT(NN,mu*ppt); unum_nodept = MTmatrix*unum_coeff;
...
loglog(NNN,10^-1*NNN.^(-1.5-3*s),'--'), hold on
loglog(NNN,10^-1*NNN.^(-0.75-3*s),'--'), hold on
legend('Linf-error','L2-error','O(N^{-1.5-3s})','O(N^{-0.75-3s})')
