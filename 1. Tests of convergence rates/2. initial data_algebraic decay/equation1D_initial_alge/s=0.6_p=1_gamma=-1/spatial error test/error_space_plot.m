...
loglog(Line,10^(-1.1)*Line.^(-2.54),'r--','LineWidth',2), hold on
loglog(Line,10^(-1)*Line.^(-3.28),'b--','LineWidth',2)
...
axis([2^1,2^8,10^-8.95,10^0])

% Legend
h_legend = legend('$L^2$-error','$L^\infty$-error','$\propto N^{-2.54}$', ...
    '$\propto N^{-3.28}$','Interpreter', 'latex');
