...
% (0.6,1,-1) and initial value function \psi_0(x,y,z) =
% 1/(1+x^2+y^2+z^2)^(1+sqrt(2)) by BioMT+TSM4, and generates the spatial
% errors about the truncation parameter N.
...
Mu = 1.5./sqrt(NNN); % the scaling factor for MT-functions, change {MT(j,x)*MT(k,y)*MT(l,z)} to {MT(j,mu*x)*MT(k,mu*y)*MT(l,mu*z)}
dt = 0.002; % time step, t_{n+1} = t_n + dt
...
loglog(NNN,10^1*NNN.^(-4.5-3*s),'--'), hold on
loglog(NNN,10^1*NNN.^(-2.25-3*s),'--'), hold on
