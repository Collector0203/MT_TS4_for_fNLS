...
loglog(Line,10^(-0.3)*Line.^(-3.15),'r--','LineWidth',2), hold on
loglog(Line,10^(0.25)*Line.^(-5.35),'b--','LineWidth',2)
...
axis([2^2,2^6,10^-9.45,10^0.3])

% Legend
h_legend = legend('$L^2$-error','$L^\infty$-error','$\propto N^{-3.15}$', ...
    '$\propto N^{-5.35}$','Interpreter', 'latex');
