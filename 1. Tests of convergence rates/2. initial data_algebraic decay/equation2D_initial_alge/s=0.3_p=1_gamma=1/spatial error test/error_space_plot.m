...
loglog(Line,10^(-0.88)*Line.^(-2.45),'r--','LineWidth',2), hold on
loglog(Line,10^(-0.78)*Line.^(-3.90),'b--','LineWidth',2)
...
axis([2^2,2^7,10^-9,10^-0.3])

% Legend
h_legend = legend('$L^2$-error','$L^\infty$-error','$\propto N^{-2.45}$', ...
    '$\propto N^{-3.90}$','Interpreter', 'latex');
