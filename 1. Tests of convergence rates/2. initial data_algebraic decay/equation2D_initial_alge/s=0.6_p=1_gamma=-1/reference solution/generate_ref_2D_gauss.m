...
% and initial value function \psi_0(x,y) = 1/(1+x^2+y^2)^(1/2+sqrt(2)).
...
mu = 1.1/sqrt(N); % the scaling factor for MT-functions, change {MT(m,x)*MT(n,y)} to {MT(m,mu*x)*MT(n,mu*y)}
dt = 0.0001; % time step, t_{n+1} = t_n + dt
