% This program computes the example presented in Figure 5(a) of "(Sheng
% 2020) Fast Fourier-like Mapped Chebyshev Spectral-Galerkin Methods for
% PDEs with Integral Fractional Laplacian in Unbounded Domains" and plot
% the spatial-error figure about truncation parameter N, demonstrating that
% the accuracy of BioMT+TSM4 is superior to that of BioMCF+TSM4.
% 
% 1. 2D nonlinear fractional Schrödinger equation: 1i*\partial_t
% \psi(x,y,t) = 1/2*(-\Delta)^s \psi(x,y,t) +
% gamma*|\psi(x,y,t)|^{2*p}*\psi(x,y,t), where (x,y) \in R^2, t>0.
% 2. equation parameters: (s,p,gamma) = (0.7,1,-1).
% 3. initial data: \psi_0(x,y) = sech(x)*sech(y)*exp(1i*(x+y)).

clear

%% Set the parameters for equations, space discretization, time discretization
s = 0.7; p = 1; gamma = -1; % equation parameter
NNN = 20:10:100; % truncation, \psi(x,y,t) = sum(m=-N:N-1)sum(n=-N:N-1) a(m,n,t)*MT(m,mu*x)*MT(n,mu*y)
Mu = 1.5./sqrt(NNN); % the scaling factor for MT-functions, change {MT(m,x)*MT(n,y)} to {MT(m,mu*x)*MT(n,mu*y)}
...
pt = -M:M/4000:M; [pt2,pt1] = meshgrid(pt); % test points [pt1,pt2]
