% This program generates the reference solution for the 2D fractional
% nonlinear Schrödinger equation with parameters (s,p,gamma) = (0.7,1,-1)
% and initial value function \psi_0(x,y) = sech(x)*sech(y)*exp(1i*(x+y)).
...
% \psi(x,y,T) with TT = [0.5,1,1.5,2].

clear

%% Set the parameters for equations, space discretization, time discretization
s = 0.7; p = 1; gamma = -1; % equation parameter
N = 300; % truncation, \psi(x,y,t) = sum(m=-N:N-1)sum(n=-N:N-1) a(m,n,t)*MT(m,mu*x)*MT(n,mu*y)
mu = 1.5/sqrt(N); % the scaling factor for MT-functions, change {MT(m,x)*MT(n,y)} to {MT(m,mu*x)*MT(n,mu*y)}
dt = 0.0002; % time step, t_{n+1} = t_n + dt
...
T = 2; % final time
TT = [0.5,1,1.5,2]; % the time that record the reference solution
...
unum_node = zeros(2*N,2*N,length(TT)); % unum_node(:,:,q) denotes the numerical values of \psi(x,y,t=TT(q)) on MT-nodes
...
N_ref = N;
dt_ref = dt;
unum_coeff_ref = zeros(2*N,2*N,length(TT)); % unum_coeff_ref(:,:,q) denotes the MT-coefficients of \psi(x,y,t=TT(q))
for k = 1:length(TT)
    unum_coeff_ref(:,:,k) = MTcoeff2(unum_node(:,:,k),N,opt_Sign_SignM,opt_iTheta);
end
