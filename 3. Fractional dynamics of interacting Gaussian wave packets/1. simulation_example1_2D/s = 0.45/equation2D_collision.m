...
% 
% 1. parameters: (s,p,gamma) = (0.45,1,1).
% 2. initial: \psi_0(x,y) = exp(-(x+3)^2-1.5*y^2)*exp(1i*5*x) +
% exp(-(x-3)^2-1.5*y^2)*exp(-1i*5*x).

clear, close all

%% Set the parameters for equations, space discretization, time discretization
s = 0.45; p = 1; gamma = 1; % equation parameter
N = 256; % truncation, \psi(x,y,t) = sum(m=-N:N-1)sum(n=-N:N-1) a(m,n,t)*MT(m,mu*x)*MT(n,mu*y)
mu = 2/sqrt(N); % the scaling factor for MT-functions, change {MT(m,x)*MT(n,y)} to {MT(m,mu*x)*MT(n,mu*y)}
...
save("simulation2D_collision_s=0.45.mat","s","p","gamma","N","mu","dt","T","unum_node","Tstep","inter","opt_Sign_SignM","opt_iTheta");