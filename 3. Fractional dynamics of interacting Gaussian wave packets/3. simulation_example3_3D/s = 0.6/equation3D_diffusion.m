...
mu = 0.5/sqrt(N); % the scaling factor for MT-functions, change {MT(j,x)*MT(k,y)*MT(l,z)} to {MT(j,mu*x)*MT(k,mu*y)*MT(l,mu*z)}
...
save("simulation3D_diffusion_s=0.6.mat","s","p","gamma","N","mu","dt","T","unum_coeff","Tstep","inter");